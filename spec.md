...
> - 重なり禁止＋移動失敗ルールを明確化
> - 観測空間をtotalistic（合計ベース）設計に変更。Phase 1（密度のみ）とPhase 2（状態プロファイル）の二段階比較実験
> - ランダム逐次更新を採用
> - 全指標の計算式を定義
...
Phase 1とPhase 2を同一条件（同一seed、同一フィルタ）で実行し、以下を比較する:
- 生存率の差
- 生存ルールの指標分布の差
- 創発構造の質的な違い
...
- **形式**: Phase 1は20エントリ、Phase 2は100エントリのルックアップテーブル。各エントリは行動ID（0〜8）。
- **インデックス計算**: セクション3の各Phaseの定義に従う。
- **生成方法**: 各エントリをランダムに0〜8から一様に選択。
- **ルール空間**: Phase 1: 9^20 ≈ 10^19、Phase 2: 9^100 ≈ 10^95。
...
| 観測 | Totalistic設計。Phase 1: 自身の状態×近傍占有数（20エントリ）、Phase 2: ＋支配的状態（100エントリ） | GoLの知見に基づく合計ベース設計。二段階比較で状態情報の必要性を検証 |
