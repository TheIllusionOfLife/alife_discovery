...
| 左セルの個体数 | 0 or 1（2値） | |
| 右セルの個体数 | 0 or 1（2値） | |
**入力空間サイズ**: 4 × 2^4 = 64 エントリ

> 注: 自セルの個体数は常に1（自分がいる）なので観測入力に含めない。近傍エージェントの内部状態は観測しない。個体の存在有無のみ。
...
- **形式**: 64エントリのルックアップテーブル。各エントリは行動ID（0〜8）。
- **インデックス計算**: `state * 16 + up * 8 + down * 4 + left * 2 + right`（state: 0-3, up/down/left/right: 0 or 1）
- **生成方法**: 各エントリをランダムに0〜8から一様に選択。
- **ルール空間**: 9^64 ≈ 10^61（膨大だがランダムサンプリングで探索）。
...
| 予測可能性 | t時点の状態からt+1の状態をどの程度予測できるか。全エージェントの内部状態ベクトルを4進数列として扱い、連続するステップ間の正規化圧縮距離（NCD）で近似する。NCD(x,y) = (C(xy) - min(C(x),C(y))) / max(C(x),C(y))、Cはzlib圧縮サイズ。値域 [0,1]、0=完全予測可能、1=完全不予測。相互情報量の直接推定は高次元でバイアスが大きいため採用しない。 |
...
| 観測 | 自身の状態＋近傍4セルの個体数 | テーブルサイズ64で管理可能 |
