...
swaps), the null model is well-mixed: a KS test confirms that the
empirical $p$-value distribution is consistent with uniformity among
non-trivial entities ($a_i > 0$).
...
observations) yields a 95\% CI of $[0.0\%, 0.0\%]$ on the overall
excess rate. A one-sided Clopper-Pearson bound confirms that if
run-level excess exists, it is ${<}\,0.1\%$ with 95\% confidence.
With 281 unique types, we would detect a true excess rate
${\geq}\,0.6\%$ at 80\% power, confirming adequate sensitivity.
