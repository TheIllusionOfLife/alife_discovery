...
this as a boundary condition: under low-expressiveness random bonding
rules, assembly complexity is entirely size-determined.
\end{abstract}
...
were observed in exhaustive checks against canonical isomorphism for all
281 observed entity types (up to 6 nodes).
...
into two disjoint sets) into two sub-objects; when $S_1 \cong S_2$
...
$|E|{-}1$ exactly for all small entities.
...
With $n_{\text{shuffle}} = 100$ (each applying $\max(4, |E|)$ double-edge
swaps), the null model is well-mixed, but a KS test on empirical
$p$-values for non-trivial entities ($a_i > 0$) rejects strict
uniformity. The observed deviation is concentrated at conservative
high-$p$ mass (many ties near $p{=}1$), consistent with the discrete null
in the small-entity regime, and does not produce any excess positives.
...
Context-weighted averaging (weighting by observation frequency per
context) yields a similar estimate. Death rates $d_k$ increase with $k$
...
bonding. Testable hypotheses include: (i)~partner-specific rules
(conditioning on partner type) could enable motif-selective bonding;
(ii)~spatially-oriented rules could produce direction-dependent
structures; and (iii)~rule evolution could discover structure-promoting
parameter regions. These extensions would help disentangle rule
expressiveness from randomness as factors in the negative result.
...
entity observations, parameter sweeps over 11 conditions, and a
...
\citep{fanelli2012negativeresults}---establishes that, under
low-expressiveness random bonding rules, emergent structural complexity
does not arise from local interactions alone. Structural bias in the rules---whether
through motif-selective bonding, partner-specific conditions, or
adaptation---appears necessary even without explicit fitness functions.
By characterizing this boundary condition for a specific rule class, we
provide a concrete experimental baseline against which richer
rule representations and objective-free interventions can be measured.
